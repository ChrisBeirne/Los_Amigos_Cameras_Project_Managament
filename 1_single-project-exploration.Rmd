...
Summary of the currently deployed features cameras:
```{r}
table(active.deps[active.deps$Subprojet=="Features",]$Type.of.station)
...
```{r, echo=F, include=F}
tmp <- active.deps[active.deps$Subprojet=="Features",]

# Generate colours to display the category levels - R needs them as a factor
tmp[,category] <- factor(tmp[,"Type.of.station"])
col.cat <- wheel(colour, num = length(levels(tmp[,category])))
tmp$Cols <- col.cat[tmp[,category]]
```

```{r map features, echo=F}


# Count the number of camera ststions
n.stat <- length(unique(tmp$deployment_id))

m <- leaflet() %>%
  addProviderTiles(providers$Esri.WorldTopoMap, group="Base") %>%
  addProviderTiles(providers$Esri.WorldImagery, group="Satellite") %>%  # Add satellite data
  
  addCircleMarkers(lng= tmp$Longitude, lat= tmp$Latitude,
                   color= tmp$Cols,
                   popup=paste( tmp$deployment_location_id,  tmp[,category]), radius=3) %>%
 addLegend("topleft", colors = col.cat,  labels = levels( tmp[,category]),
    title = category,
    labFormat = labelFormat(prefix = "$"),
    opacity = 1
  ) %>%
  ...
  addMarkers(
    lng = tmp$Longitude, lat = tmp$Latitude,
    label = tmp$deployment_location_id,
...
st_write(feats_sf, "data/shapefiles/Feature_locations.shp")
