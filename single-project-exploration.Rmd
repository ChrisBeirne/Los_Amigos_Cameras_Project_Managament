...
The project currently has `r nrow(all.deps[all.deps$End.date=="",])` cameras active in the field. To date the labelling platform contains `nrow(dep)` completed deployments from `r length(unique(dep$subproject_name))` sub-project(s).  

The deployments currently span `r paste(round(as.numeric(st_area(st_as_sfc(st_bbox(tmp)))/1e+6),1), "km^2")`
...
p1 <- plot_ly(
  ...
  layout(plot_bgcolor = "#f5f5f5", 
         ...
         yaxis = list(title = 'Total deployments to date'),
...
# Camera status at the end of deployment
...
```{r, echo=F}
# Plot the camera activity profiles
